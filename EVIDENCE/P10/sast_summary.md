...
- Bootstrap-пароли вынесены в `.env`/`tests/conftest.py` и хешируются, поэтому текущее сканирование (`docker run ... detect --no-git`) создаёт пустой отчёт (`EVIDENCE/P10/gitleaks.json`).
